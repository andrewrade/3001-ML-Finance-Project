...
def preprocesser(raw_df, preproc_params, new=True, interest_rates=True):
    ...
    if interest_rates:
        ...
        interest_rates['DATE'].dt.strftime("%Y-%m-%d")
        ...
        df = pd.merge_asof(df.sort_values('stmt_date'), interest_rates.sort_values('offset_date'), left_on='stmt_date', right_on='offset_date', direction='nearest', tolerance=pd.Timedelta(seconds=10))

    # Convert Legal Structure & ATECO Code to categorical fields 
    df['legal_struct'] = pd.Categorical(df['legal_struct'])
    df['legal_structb'] = df['legal_struct'].cat.codes
